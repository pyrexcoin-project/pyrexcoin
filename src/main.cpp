...
#if defined(NDEBUG)
# error "Pyrexcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x90d5f1e5fd93d26932bb31f6e0fa4a504f91d0b83d9cb0bd4880646cf7e44446");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pyrexcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pyrexcoin Signed Message:\n";
...
{
    // Pyrexcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Pyrexcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Pyrexcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Pyrexcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pyrexcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pyrexcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x90d5f1e5fd93d26932bb31f6e0fa4a504f91d0b83d9cb0bd4880646cf7e44446");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NEXT GENERATION CRYPTOCURRENCY";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("787784710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1512991894;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1071851;
        ...
        assert(block.hashMerkleRoot == uint256("0x069d6de48c34cd243bf5d7c81f8076474259e2669dbaff279ab9ec6710b9350c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Pyrexcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PyrexcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PyrexcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PyrexcoinMiner(CWallet *pwallet)
{
    printf("PyrexcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pyrexcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PyrexcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PyrexcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PyrexcoinMiner, pwallet));
}
